...
export class Repository {
	...
	async stripCommitMessageComments(message: string): Promise<string> {
		// Get the comment character from git config, defaulting to '#'
		let commentChar = '#';
		try {
			const result = await this.exec(['config', '--get', 'core.commentChar']);
			if (result.stdout) {
				commentChar = result.stdout.trim();
			}
		} catch {
			// If config is not set or fails, use default '#'
		}

		// Escape special regex characters
		const escapedChar = commentChar.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
		const regex = new RegExp(`^\\s*${escapedChar}.*$\\n?`, 'gm');
