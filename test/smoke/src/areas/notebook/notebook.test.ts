...
export function setup(logger: Logger) {
	describe('Notebooks', () => { // https://github.com/microsoft/vscode/issues/140575
		...
		it.skip('moves focus in and out of output', async function () { // Disabled due to flakiness (see closed issue #139270)
			const app = this.app as Application;
			await app.workbench.notebook.openNotebook();
		...
		it.skip('cell action execution', async function () { // Disabled due to flakiness (see closed issue #139270)
			const app = this.app as Application;
			await app.workbench.notebook.openNotebook();
			await app.workbench.notebook.insertNotebookCell('code');
