...
/* eslint-disable */ // Void
import { GRID_CTRL_K_ACTION_ID, GRID_CTRL_L_ACTION_ID } from '../../../contrib/grid/browser/actionIDs.js';
...
export class EditorGroupWatermark extends Disposable {
	...
	constructor(
		...
		// GRID text-based placeholder (no branding visuals yet)
		const updateTheme = () => {
			const theme = this.themeService.getColorTheme().type;
			const textColor = theme === ColorScheme.DARK || theme === ColorScheme.HIGH_CONTRAST_DARK
				? 'rgba(255, 255, 255, 0.15)'
				: 'rgba(0, 0, 0, 0.15)';
			// Display GRID text placeholder
	...
	private render(): void {
		...
		const update = async () => {
			...
			if (this.contextService.getWorkbenchState() === WorkbenchState.EMPTY) {
				...
				const keys = this.keybindingService.lookupKeybinding(GRID_CTRL_L_ACTION_ID);
				...
				const clickChatHandler = () => {
					...
					this.commandService.executeCommand(GRID_CTRL_L_ACTION_ID).catch(err => {
				...
				const keys2 = this.keybindingService.lookupKeybinding(GRID_CTRL_K_ACTION_ID);
				...
				const clickQuickEditHandler = () => {
					...
					this.commandService.executeCommand(GRID_CTRL_K_ACTION_ID).catch(err => {
				...
				// 	label3.set(keys3);
				// button3.onclick = () => {
				// 	this.commandService.executeCommand(GRID_OPEN_SETTINGS_ACTION_ID)
				// }
