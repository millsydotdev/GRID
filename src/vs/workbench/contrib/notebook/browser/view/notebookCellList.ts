...
export class NotebookCellList extends WorkbenchList<CellViewModel> implements IDisposable, IStyleController, INotebookCellList {
	...
	attachViewModel(model: NotebookViewModel) {
		...
		this._viewModelStore.add(model.onDidChangeSelection((e) => {
			...
			// Optimized: single pass instead of chained map-filter-map
			const viewSelections: number[] = [];
			for (const index of cellRangesToIndexes(model.getSelections())) {
				const cell = model.cellAt(index);
				if (cell) {
					viewSelections.push(this._getViewIndexUpperBound(cell));
				}
			}
			this.setSelection(viewSelections, undefined, true);
			// Optimized: single pass instead of chained map-filter-map
			const primary: number[] = [];
			for (const index of cellRangesToIndexes([model.getFocus()])) {
				const cell = model.cellAt(index);
				if (cell) {
					primary.push(this._getViewIndexUpperBound(cell));
				}
			}
