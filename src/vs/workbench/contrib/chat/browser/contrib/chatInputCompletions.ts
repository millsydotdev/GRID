...
class BuiltinDynamicCompletions extends Disposable {
	...
	// Optimized: cache RegExp for file/symbol completions to avoid recreating
	private static readonly FileWordPattern = new RegExp(`${chatVariableLeader}[^\\s]*`, 'g');
	...
	constructor(
		...
		}, BuiltinDynamicCompletions.FileWordPattern);
		...
		this.registerVariableCompletions('symbol', ({ widget, range, position, model }, token) => {
			...
			const result: CompletionList = { suggestions: [] };
			const range2 = computeCompletionRanges(model, position, BuiltinDynamicCompletions.FileWordPattern, true);
			if (range2) {
