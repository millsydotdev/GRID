...
import { MCPUserStateOfName } from '../common/gridSettingsTypes.js';
	...
	private async _createClientUnsafe(
		...
		if (server.url) {
			...
			try {
				...
				throw new Error(
					`Invalid URL for server ${serverName}: ${server.url}. ${urlErr instanceof Error ? urlErr.message : String(urlErr)}`
				);
			}
			...
			if (transportType === 'sse') {
				try {
					...
					throw new Error(
						`Failed to connect to SSE server at ${urlString}: ${sseErr instanceof Error ? sseErr.message : String(sseErr)}`
					);
				}
			}
			// If type is explicitly 'http', only try HTTP
				...
				try {
					...
					throw new Error(
						`Failed to connect to HTTP server at ${urlString}: ${httpErr instanceof Error ? httpErr.message : String(httpErr)}`
					);
				}
			}
