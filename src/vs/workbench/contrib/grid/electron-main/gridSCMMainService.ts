...
export class GridSCMService implements IGridSCMService {
	...
	async gitSampledDiffs(path: string): Promise<string> {
		...
		const topFiles = numStatList.sort((a, b) => b.added + b.removed - (a.added + a.removed)).slice(0, MAX_DIFF_FILES);
		const diffs = await Promise.all(
			topFiles.map(async ({ file }) => ({ file, diff: await getSampledDiff(file, path, useStagedChanges) }))
		);
