...
export const IDirectoryStrService = createDecorator<IDirectoryStrService>('gridDirectoryStrService');
		...
		if (child.isDirectory && !isGitIgnoredDirectory) {
			...
			if (eChildren && eChildren.length > 0) {
				const { childrenContent: grandChildrenContent, childrenCutOff: grandChildrenCutOff } =
					await renderChildrenCombined(eChildren, remainingChars, nextLevelPrefix, fileService, fileCount, {
						maxDepth,
