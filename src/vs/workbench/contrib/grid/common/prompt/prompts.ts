...
export const builtinTools: {
	...
	run_nl_command: {
		...
		params: {
			nl_input: {
				description:
					'Natural language description of the command to run (e.g., "list git branches", "run npm tests", "check current directory").',
			},
			cwd: { description: cwdHelper },
		},
	},
...
- Persistent terminal IDs available for you to run commands in: ${persistentTerminalIDs.join(', ')}`
	...
	const truncatedDirStr =
		directoryStr.length > MAX_DIRSTR_LENGTH
...
export const chat_systemMessage_local = ({
	workspaceFolders,
	openedURIs,
	activeURI,
	chatMode: mode,
...
export const gridPrefixAndSuffix = ({
	fullFileStr,
	startLine,
	endLine,
}: {
...
export const ctrlKStream_systemMessage_local = ({
	quickEditFIMTags: { preTag, midTag, sufTag },
}: {
	quickEditFIMTags: QuickEditFimTagsType;
}) => {
...
// export const aiRegex_computeReplacementsForFile_userMessage = async ({ searchClause, replaceClause, fileURI, gridFileService }: { searchClause: string, replaceClause: string, fileURI: URI, gridFileService: IGridFileService }) => {

// 	// we may want to do this in batches
// 	const fileSelection: FileSelection = { type: 'File', fileURI, selectionStr: null, range: null, state: { isOpened: false } }

// 	const file = await stringifyFileSelections([fileSelection], gridFileService)
