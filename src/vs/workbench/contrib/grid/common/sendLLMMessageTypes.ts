...
export type GeminiLLMChatMessage =
	| {
			role: 'model';
			parts: ({ text: string } | { functionCall: { id: string; name: ToolName; args: Record<string, unknown> } })[];
	  }
	| {
			role: 'user';
			parts: (
...
export type ServiceSendLLMMessageParams = {
	...
	onError: OnError;
	logging: { loggingName: string; loggingExtras?: Record<string, unknown> };
...
export type MainModelListParams<modelResponse> = Omit<ModelListParams<modelResponse>, BlockedMainModelListParams> & {
	providerName: RefreshableProviderName;
	requestId: string;
};
...
>[0] & { requestId: string };
export type EventModelListOnErrorParams<modelResponse> = Parameters<ModelListParams<modelResponse>['onError']>[0] & {
	requestId: string;
};
