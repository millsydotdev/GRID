...
const gridSubstr = (str: string, start: number, end: number) => (end < start ? '' : str.substring(start, end));
...
export const extractSearchReplaceBlocks = (str: string) => {
	...
	while (true) {
		...
		if (dividerStart === -1) {
			...
			blocks.push({
				orig: gridSubstr(str, origStart, str.length - writingDIVIDERlen),
				final: '',
				state: 'writingOriginal',
			});
			return blocks;
		}
		const origStrDone = gridSubstr(str, origStart, dividerStart);
		...
		if (finalStart === -1) {
			...
			blocks.push({
				orig: origStrDone,
				final: gridSubstr(str, dividerStart, str.length - usingWritingFINALlen),
				state: 'writingFinal',
			});
			return blocks;
		}
		const usingFINAL = matchedFullFINAL_ ? '\n' + FINAL : FINAL;
		const finalStrDone = gridSubstr(str, dividerStart, finalStart);
