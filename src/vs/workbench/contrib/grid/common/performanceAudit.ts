...
export function generatePerformanceAuditReport(mode: 'auto' | 'single' = 'auto'): PerformanceAuditReport {
	...
	if (mode !== 'auto' && metrics.tokensPerSecond.p50 < tokensPerSecondTarget) {
		bottlenecks.push(
			`Throughput low: ${metrics.tokensPerSecond.p50.toFixed(1)} tokens/s (target: ≥${tokensPerSecondTarget})`
		);
	}
...
export function printPerformanceAuditReport(mode: 'auto' | 'single' = 'auto'): void {
	...
	console.log(
		`TTFS ≤${report.targets.ttfs.target}ms: ${report.targets.ttfs.met ? '✅' : '❌'} (${report.metrics.ttfs.p95.toFixed(1)}ms)`
	);
	console.log(
		`TTS ≤${report.targets.tts.target}ms: ${report.targets.tts.met ? '✅' : '❌'} (${report.metrics.tts.p95.toFixed(1)}ms)`
	);
	if (report.metrics.routerDecisionTime.p50 > 0) {
		console.log(
			`Router ≤${report.targets.routerDecisionTime.target}ms: ${report.targets.routerDecisionTime.met ? '✅' : '❌'} (${report.metrics.routerDecisionTime.p95.toFixed(1)}ms)`
		);
	}
	if (report.targets.tokensPerSecond.target > 0) {
		console.log(
			`Tokens/s ≥${report.targets.tokensPerSecond.target}: ${report.targets.tokensPerSecond.met ? '✅' : '❌'} (${report.metrics.tokensPerSecond.p50.toFixed(1)})`
		);
	}
	console.groupEnd();
