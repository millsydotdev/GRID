...
const extensionBlacklist = [
	...
	// ignore other AI copilots that could conflict with GRID keybindings
	'sourcegraph.cody-ai',
	'continue.continue',
	'codeium.codeium',
		...
		if (fromEditor === 'VS Code') {
			return [
				...
				},
				{
					...
					to: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, '.grid-editor', 'extensions'),
					isExtensions: true,
				},
			];
			...
			return [
				{
					...
					to: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, '.config', 'GRID', 'User', 'settings.json'),
				},
				{
					from: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, '.config', 'Code', 'User', 'keybindings.json'),
					to: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, '.config', 'GRID', 'User', 'keybindings.json'),
				},
				{
					from: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, '.vscode', 'extensions'),
					to: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, '.grid-editor', 'extensions'),
			...
			return [
				{
					...
					to: URI.joinPath(URI.from({ scheme: 'file' }), appdata, 'GRID', 'User', 'settings.json'),
				},
				{
					from: URI.joinPath(URI.from({ scheme: 'file' }), appdata, 'Code', 'User', 'keybindings.json'),
					to: URI.joinPath(URI.from({ scheme: 'file' }), appdata, 'GRID', 'User', 'keybindings.json'),
				},
				{
					from: URI.joinPath(URI.from({ scheme: 'file' }), userprofile, '.vscode', 'extensions'),
					to: URI.joinPath(URI.from({ scheme: 'file' }), userprofile, '.grid-editor', 'extensions'),
					isExtensions: true,
				},
			];
		} else if (fromEditor === 'Cursor') {
