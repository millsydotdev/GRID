...
class FilePromptActionService extends Action2 {
	private static readonly GRID_COPY_FILE_PROMPT_ID = 'grid.copyfileprompt';

	constructor() {
		super({
			id: FilePromptActionService.GRID_COPY_FILE_PROMPT_ID,
			title: localize2('gridCopyPrompt', 'GRID: Copy Prompt'),
			menu: [
				{
				...
				m = await messageOfSelection(
					{
						type: 'File',
						uri,
						language: (await gridModelService.getModelSafe(uri)).model?.getLanguageId() || '',
