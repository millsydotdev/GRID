...
const notifyUpdate = (
	res: GridCheckUpdateResponse & { message: string },
	notifService: INotificationService,
	updateService: IUpdateService
): INotificationHandle => {
	const message = res?.message || 'This is a very old version. Please download the latest GRID!';
...
const notifyErrChecking = (notifService: INotificationService): INotificationHandle => {
	const message = `There was an error checking for updates. If this persists, please reinstall GRID.`;
	const notifController = notifService.notify({
		severity: Severity.Info,
		message: message,
		sticky: true,
...
registerAction2(
	class extends Action2 {
		...
		async run(accessor: ServicesAccessor): Promise<void> {
			...
			const newController = await performGridCheck(
				true,
				notifService,
				gridUpdateService,
				metricsService,
...
class GridUpdateWorkbenchContribution extends Disposable implements IWorkbenchContribution {
	static readonly ID = 'workbench.contrib.grid.gridUpdate';
	constructor(
...
registerWorkbenchContribution2(
	GridUpdateWorkbenchContribution.ID,
	GridUpdateWorkbenchContribution,
