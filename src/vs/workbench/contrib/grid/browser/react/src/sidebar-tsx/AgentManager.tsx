...
import { X, Plus, Search, MessageCircle, Folder, Settings, Sparkles, Pin, Clock, Trash2, Copy, Inbox, Book, Globe, MessageSquare, ChevronRight, ChevronDown, PenTool, Database, BarChart2, ThumbsUp, ThumbsDown } from 'lucide-react';
...
import { gridLearningEngine } from '../../../../common/gridLearningEngine.js';

interface AgentManagerProps {
	onClose: () => void;
}

type SidebarView = 'inbox' | 'workspaces' | 'composer' | 'stats';
			...
			<div className="grid-w-[95%] grid-max-w-7xl grid-h-[90%] grid-bg-grid-bg-0 grid-rounded-2xl grid-shadow-2xl grid-border grid-border-grid-border-2 grid-flex grid-overflow-hidden">
				...
				<div className="grid-w-72 grid-bg-grid-bg-1 grid-border-r grid-border-grid-border-2 grid-flex grid-flex-col">
					...
					<button
						...
						className={`grid-mx-3 grid-mt-2 grid-px-3 grid-py-2 grid-rounded-lg grid-flex grid-items-center grid-gap-2 grid-text-sm grid-transition-all ${sidebarView === 'stats'
							? 'grid-bg-grid-primary-soft grid-text-grid-primary'
							: 'grid-text-grid-fg-2 hover:grid-bg-grid-bg-2 hover:grid-text-grid-fg-1'
							}`}
					>
						<BarChart2 className="grid-w-4 grid-h-4" />
						Performance
					</button>
					...
					<div className="grid-px-6 grid-py-4 grid-border-b grid-border-grid-border-2 grid-flex grid-items-center grid-justify-between">
						<div className="grid-flex grid-items-center grid-gap-4">
							<h2 className="grid-text-lg grid-font-semibold grid-text-grid-fg-0">
								{sidebarView === 'inbox' ? 'Inbox' : sidebarView === 'composer' ? 'Composer' : sidebarView === 'stats' ? 'Agent Performance' : 'Conversations'}
							</h2>
							{sidebarView === 'inbox' && (
					...
					<div className="grid-flex-1 grid-overflow-y-auto grid-p-6">
						{sidebarView === 'inbox' ? (
							...
							<div className="grid-space-y-6">
								{(() => {
									const stats = gridLearningEngine.getFeedbackStats();
									const total = stats.positive + stats.negative;
									const satisfaction = total > 0 ? Math.round((stats.positive / total) * 100) : 0;

									return <>
										{/* Overview Cards */}
										<div className="grid-grid grid-grid-cols-3 grid-gap-4">
											<div className="grid-p-4 grid-bg-grid-bg-1 grid-rounded-xl grid-border grid-border-grid-border-2">
												<div className="grid-flex grid-items-center grid-gap-2 grid-mb-2 grid-text-grid-fg-3">
													<ThumbsUp className="grid-w-4 grid-h-4" />
													<span className="grid-text-xs grid-font-semibold grid-uppercase">Positive Feedback</span>
												</div>
												<div className="grid-text-2xl grid-font-bold grid-text-grid-fg-0">{stats.positive}</div>
											</div>
											<div className="grid-p-4 grid-bg-grid-bg-1 grid-rounded-xl grid-border grid-border-grid-border-2">
												<div className="grid-flex grid-items-center grid-gap-2 grid-mb-2 grid-text-grid-fg-3">
													<ThumbsDown className="grid-w-4 grid-h-4" />
													<span className="grid-text-xs grid-font-semibold grid-uppercase">Negative Feedback</span>
												</div>
												<div className="grid-text-2xl grid-font-bold grid-text-grid-fg-0">{stats.negative}</div>
											</div>
											<div className="grid-p-4 grid-bg-grid-bg-1 grid-rounded-xl grid-border grid-border-grid-border-2">
												<div className="grid-flex grid-items-center grid-gap-2 grid-mb-2 grid-text-grid-fg-3">
													<Sparkles className="grid-w-4 grid-h-4" />
													<span className="grid-text-xs grid-font-semibold grid-uppercase">Satisfaction Rate</span>
												</div>
												<div className="grid-text-2xl grid-font-bold grid-text-grid-fg-0">{satisfaction}%</div>
												<div className="grid-w-full grid-h-1.5 grid-bg-grid-bg-3 grid-rounded-full grid-mt-2 grid-overflow-hidden">
													<div className="grid-h-full grid-bg-grid-success" style={{ width: `${satisfaction}%` }} />
												</div>
											</div>
										</div>

										{/* Breakdown by Mode */}
										<div className="grid-bg-grid-bg-1 grid-rounded-xl grid-border grid-border-grid-border-2 grid-overflow-hidden">
											<div className="grid-px-4 grid-py-3 grid-border-b grid-border-grid-border-2">
												<h3 className="grid-text-sm grid-font-semibold grid-text-grid-fg-0">Breakdown by Agent Mode</h3>
											</div>
											<div className="grid-divide-y grid-divide-grid-border-2">
												{Object.entries(stats.byMode).map(([mode, modeStats]) => {
													const modeTotal = modeStats.positive + modeStats.negative;
													const modeSat = modeTotal > 0 ? Math.round((modeStats.positive / modeTotal) * 100) : 0;
													return (
														<div key={mode} className="grid-p-4 grid-flex grid-items-center grid-justify-between">
															<div className="grid-flex grid-items-center grid-gap-3">
																<div className="grid-px-2 grid-py-1 grid-bg-grid-bg-2 grid-rounded grid-text-xs grid-font-mono grid-uppercase grid-font-bold grid-text-grid-fg-2">
																	{mode}
																</div>
																<div className="grid-text-sm grid-text-grid-fg-3">
																	{modeTotal} responses
																</div>
															</div>
															<div className="grid-flex grid-items-center grid-gap-6">
																<div className="grid-flex grid-items-center grid-gap-2 grid-text-xs">
																	<span className="grid-text-grid-success">{modeStats.positive} 👍</span>
																	<span className="grid-text-grid-fg-4">/</span>
																	<span className="grid-text-grid-danger">{modeStats.negative} 👎</span>
																</div>
																<div className="grid-w-24 grid-flex grid-items-center grid-gap-2">
																	<div className="grid-flex-1 grid-h-1.5 grid-bg-grid-bg-3 grid-rounded-full grid-overflow-hidden">
																		<div className="grid-h-full grid-bg-grid-primary" style={{ width: `${modeSat}%` }} />
																	</div>
																	<span className="grid-text-xs grid-font-medium grid-text-grid-fg-1 w-8 text-right">{modeSat}%</span>
																</div>
															</div>
														</div>
													);
