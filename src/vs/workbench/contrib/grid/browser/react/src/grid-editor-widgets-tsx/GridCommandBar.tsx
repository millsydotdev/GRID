...
import { GridCommandBarProps } from '../../../gridCommandBarService.js';
...
export const GridCommandBarMain = ({ uri, editor }: GridCommandBarProps) => {
	const isDark = useIsDark()

	return <div
		className={`@@grid-scope ${isDark ? 'dark' : ''}`}
	>
		<GridCommandBar uri={uri} editor={editor} />
	</div>
}
...
export const GridCommandBar = ({ uri, editor }: GridCommandBarProps) => {
	...
	const gridModelService = accessor.get('IGridModelService')
	...
	const _upKeybinding = keybindingService.lookupKeybinding(GRID_GOTO_PREV_DIFF_ACTION_ID);
	const _downKeybinding = keybindingService.lookupKeybinding(GRID_GOTO_NEXT_DIFF_ACTION_ID);
	const _leftKeybinding = keybindingService.lookupKeybinding(GRID_GOTO_PREV_URI_ACTION_ID);
	const _rightKeybinding = keybindingService.lookupKeybinding(GRID_GOTO_NEXT_URI_ACTION_ID);
	const _acceptFileKeybinding = keybindingService.lookupKeybinding(GRID_ACCEPT_FILE_ACTION_ID);
	const _rejectFileKeybinding = keybindingService.lookupKeybinding(GRID_REJECT_FILE_ACTION_ID);
	const _acceptAllKeybinding = keybindingService.lookupKeybinding(GRID_ACCEPT_ALL_DIFFS_ACTION_ID);
	const _rejectAllKeybinding = keybindingService.lookupKeybinding(GRID_REJECT_ALL_DIFFS_ACTION_ID);
	...
	if (currFileIdx === null) {
		return (
			<div className="pointer-events-auto">
				<div className="flex bg-grid-bg-2 shadow-md border border-grid-border-2 [&>*:first-child]:pl-3 [&>*:last-child]:pr-3 [&>*]:border-r [&>*]:border-grid-border-2 [&>*:last-child]:border-r-0">
		...
		<div className="pointer-events-auto">
			...
			{showAcceptRejectAllButtons && showAcceptRejectAll && (
				<div className="flex justify-end mb-1">
					<div className="inline-flex bg-grid-bg-2 rounded shadow-md border border-grid-border-2 overflow-hidden">
						<div className="flex items-center [&>*]:border-r [&>*]:border-grid-border-2 [&>*:last-child]:border-r-0">
			...
			<div className="flex items-center bg-grid-bg-2 rounded shadow-md border border-grid-border-2 [&>*:first-child]:pl-3 [&>*:last-child]:pr-3 [&>*]:px-3 [&>*]:border-r [&>*]:border-grid-border-2 [&>*:last-child]:border-r-0">
