	...
	setTimeout(() => {
		try {
			const pathToCssFile = findDesiredPathFromLocalPath(
				'./src/vs/workbench/contrib/grid/browser/react/src2/styles.css',
				__dirname
			);
...
if (isWatch) {
	...
	if (!fs.existsSync('src2')) {
		try {
			...
			execSync('npx scope-tailwind ./src -o src2/ -s grid-scope -c styles.css -p "grid-"', { stdio: 'inherit' });
	...
	const scopeTailwindWatcher = spawn('npx', [
		...
		'ts,tsx,css',
		'--exec',
		'npx scope-tailwind ./src -o src2/ -s grid-scope -c styles.css -p "grid-"',
	]);
