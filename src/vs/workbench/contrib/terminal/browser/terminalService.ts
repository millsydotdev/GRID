...
export class TerminalService extends Disposable implements ITerminalService {
	...
	private _saveState(): void {
		...
		// Optimized: single pass instead of chained map-filter-map-filter
		const background: number[] = [];
		for (const bg of this._backgroundedTerminalInstances) {
			const instance = bg.instance;
			if (instance.shellLaunchConfig.forcePersist) {
		...
		const state: ITerminalsLayoutInfoById = { tabs, background };
