...
# GRID AppImage Creation Script

This script automates the process of creating an AppImage for the GRID editor using Docker. It works on macOS and Linux platforms.
## Requirements
...
3.  **Copy Required Files:**
    ...
    *   `create_appimage.sh`
    *   `grid.desktop` (GRID branded desktop entry)
    *   `grid.png` (GRID icon)
...
5.  **Result:**
    ...
    After the script completes, it will generate an AppImage named `GRID-x86_64.AppImage` (or similar, depending on your architecture) in the current directory.
...
*   **AppImage Creation:**
    *   Creates the `GRID.AppDir` structure.
    *   Copies binaries, resources, and the `.desktop` entry.
    *   Copies `grid.desktop` (GRID branded) and icon file.
